...
import { getJobs, getInternshipsByQuery, getTitleAndLocation } from '@/actions/Job';
...
const Internships = () => {
    const { jobs, jobLoading, error, hasNextPage, currentPage } = useSelector(store => store.job)
    ...
    useEffect(() => {
        if (inView && hasNextPage && !jobLoading) {
            dispatch(getInternshipsByQuery({
                keywords: keyword,
                location: location1,
                salary,
                page: currentPage + 1,
    ...
    const handleFilterApply = () => {
        ...
        dispatch(getInternshipsByQuery({ keywords: keyword, location: location1, salary, page: 1 }));
    };
    ...
    const resetFilter = () => {
        ...
        setFilterByLocation([]);
        dispatch(getJobs());
        if (keyword) searchParams1.set('keyword', null);
        if (location1) searchParams1.set('location', null);
        if (salary1) searchParams1.set('salary', null);
        setSearchParams(searchParams1);
    };

    const handleTitleChange = (titleFiltersArray) => {
        const titleFilters = titleFiltersArray.map((o) => o.value);
    ...
    const handleLocationChange = (locationFiltersArray) => {
        const locationFilters = locationFiltersArray.map((o) => o.value);
    ...
    const title = keyword || location1 || salary1
        ? `Total results found (${jobs?.length})`
        ...
        <div className='w-full internship relative'>
            <div className='flex w-full sm:px-12 relative'>
                ...
                <div className="w-full lg:w-[80%] overflow-y-auto px-5 py-5">
                    <h1
                        className='flex lg:hidden px-2 py-1 w-40 mx-auto rounded-full border-2 justify-center items-center gap-1 mb-10 text-xl font-bold text-slate-300'
                    ...
                    <div className='flex flex-wrap justify-start items-center gap-5'>
                        {jobs?.map((job) => {
                            const logoUrl = job?.company?.logo?.url ||
                                "https://img.freepik.com/free-vector/silhouette-skyline-illustration_53876-78786.jpg?w=740&t=st=1726374290~exp=1726374890~hmac=9f666b819e3b500470264a0ad0d2281b58b4da7831d9487f3e9e84934b6dbe9b";
                            return (
                                <JobCart
                                    key={job._id}
                                    date={job.createdAt}
                        ...
                        {!jobLoading && jobs?.length === 0 && (
                            <div className='w-full flex justify-center items-center flex-col'>
                                ...
                                <h1 className='font-semibold text-xl'>Couldn't find your search!</h1>
                            </div>
                        )}
                        ...
                        {!hasNextPage && jobs?.length > 0 && (
                            <div className='w-full py-6 text-center text-slate-400 text-sm'>
                                You've seen all {jobs.length} internships
                            </div>
                        )}
                    </div>
                         {jobLoading && <Loader2 />}
